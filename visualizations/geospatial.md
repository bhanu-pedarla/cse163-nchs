...
![election image](./images/geo_demo_clip.jpg)
...
![election data](./images/geo_elect_data.jpg)
```python
...
:class: dropdown
![election image](./images/geo_inset_no_borders.jpg)
```python
...
![election image](./images/geo_elect.jpg)
...
* When modifying the GeoDataFrame, we used the [`at` method](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.at.html) instead of `loc`. Using loc results in an error: "ValueError: Must have equal len keys and value when setting with an iterable".
...
![election image](./images/geo_elect_annotated.jpg)
...
![election image](./images/geo_correlation.jpg)
...
![Geospacial & Linear plots](./images/geo_line_correlations.jpg)
...
![election data](./images/geo_abc_data.jpg)
...
using Python's string [formatting](https://docs.python.org/3/library/string.html#format-string-syntax) functionality as built into Python's [f-string](https://realpython.com/python-f-strings/). The API is a bit complicated to understand. It is similar
...
We could have customized the spacing across all the subplots by using [matplotlib GridSpec](https://matplotlib.org/stable/gallery/userdemo/demo_gridspec03.html). But, since that code is pretty foreign and not
...
You can still do some geodataframe-looking plots. For example, the [Hurricane Florence plot](https://cse163.github.io/book/module-7-geospatial-data/lesson-20-dissolve-join/hurricane-florence/HurricanFlorence.html) is replicated on Hurricane Katrina using [this code](https://scitools.org.uk/cartopy/docs/latest/gallery/lines_and_polygons/hurricane_katrina.html#sphx-glr-gallery-lines-and-polygons-hurricane-katrina-py).
...
![election image](./images/geo_cartopy.jpg)
...
The [Projection types](https://scitools.org.uk/cartopy/docs/latest/reference/projections.html) are many. One popular projection is: `ccrs.PlateCarree()`.  
...
There is a lot to know about [Cartopy](https://scitools.org.uk/cartopy/docs/latest/getting_started/index.html). It can take a lot of time to learn (as with every library). So, be sure this library adds value and is appropriate for your task before you simply start using it.
